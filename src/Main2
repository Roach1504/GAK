...
public class Main {
    ...
    static void bubbleSort2(ArrayList<Integer> arr){
        int c;
    ...
    static void bubbleSortAlfavit(ArrayList<String> arr){
        String c;
        for(int i=0; i<arr.size()-1;) {
    ...
    //проверка на обратность слова (пример кот, ток)
    static  HashMap<String,String> reversString(ArrayList<String> arr){
        ArrayList<String> arr2 = new  ArrayList<>();
        ArrayList<HashMap> rez = new  ArrayList<>();
        HashMap<String,String> rez2 = new HashMap<>();
        for(String i : arr){
            char[]chars = i.toCharArray();
            int count = 0;
            char[]chars1 = new char[chars.length];
            for(int j = chars.length-1; j>=0; j--){
                chars1[count] = chars[j];
                count++;
            }
            arr2.add(String.valueOf(chars1));
        }
        for(int i = 0; i<arr.size(); i++){
            for(int j = i+1; j<arr2.size(); j++){
                if (arr.get(i).compareTo(arr2.get(j)) == 0) {
                    rez2.put(arr.get(i),arr.get(j));
                }
            }
        }
        //построчный вывод хешмепа
        /*for (Map.Entry entry : rez2.entrySet()) {
            System.out.println(entry.getKey()+" " +entry.getValue());
        }*/
        return rez2;
    }
    //удаление повторяющихся букв в слове
    static String delLetter(String s){
        char[] chars = s.toCharArray();
        HashSet<String> gg= new HashSet<String>();
        for(char i : chars){
            gg.add(String.valueOf(i));
        }
        String ss = String.join("",gg);
        return ss;
    }

    //удаление заданной буквы из слова
    static String delLetterOne(String s, String a){
        char[] chars = s.toCharArray();
        char as =  a.charAt(0);
        ...
        String ss = String.join("",rez);
        return ss;
    }

    //вставка подстроки в строку в заданное место
    static String insertString(int a, String sl, String s2){
        StringBuilder rez = new StringBuilder(sl);
        return String.valueOf(rez.insert(a, s2));
    }
...
//        ff.add("vda");
//        ff.add("asdasdasd");
//        for (Map.Entry entry : reversString(ff).entrySet()) {
//            System.out.println(entry.getKey() + " " + entry.getValue());
//        }
//        String[]sdd = {"fgd","dfgdf","fdgdf"};
//        String df = String.join("", sdd);
        StringBuilder fgdfg = new StringBuilder("faaf");
        fgdfg.insert(0,"pidr");
